...
load_list = [
    os.path.join(pd, 'src', 'roipool3d.cpp'),
    os.path.join(pd, 'src', 'roipool3d_kernel.cu')
]
roipool3d_cuda = load('roipool3d_cuda', load_list, verbose=False)
...
def roipool3d_gpu(pts,
    ...
    pooled_boxes3d = kitti.enlarge_box3d(boxes3d.view(-1, 7),
...
def pts_in_boxes3d_cpu(pts, boxes3d):
    ...
    if not pts.is_cuda:
        ...
        boxes3d = boxes3d[:, :7].float().contiguous()
...
def roipool3d_cpu(boxes3d,
    ...
    :return:
    """
    pooled_boxes3d = kitti.enlarge_box3d(boxes3d, pool_extra_width)
    ...
    if canonical_transform:
        ...
        for k in range(sampled_pts_input.shape[0]):
            sampled_pts_input[k] = kitti.rotate_pc_along_y(
