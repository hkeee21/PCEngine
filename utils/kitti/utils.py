...
def objs_to_boxes3d_with_classes(obj_list, classes):
    """
    convert KITTI object to bbox params.
    :params: obj_list: dict of list of objectss
    classes: list of str
    :return: dict of bbox params
    """
    boxes3d_dict = {}
    for c in classes:
        boxes3d = np.zeros((obj_list[c].__len__(), 7), dtype=np.float32)
        for k, obj in enumerate(obj_list[c]):
            boxes3d[k, 0:3] = obj.pos
            boxes3d[k, 3:7] = [obj.h, obj.w, obj.l, obj.ry]
        boxes3d_dict[c] = boxes3d
    return boxes3d_dict


def get_camera_fov_points(pts_rect, reduce_by_range=False,
                   area_scope=None, calib=None, img_shape=None):
    """
    Calculates the indices for points within camera FoV.
    We assume pts_rect are transformed to rect. coords.
    :params: pts_rect: (N, 3),
    reduce_by_range: bool,
    area_scope: (3, 2),
    calib: calibrator class
    image_shape: (2)
    :return: pts_valid_flag: (N), boolean array
    """
    if calib is not None:
        ...
        pts_valid_flag = np.logical_and(val_flag_merge,
                                        pts_rect_depth >= 0)
    else:
        pts_valid_flag = np.ones([pts_rect.shape[0]], 'bool')

    if reduce_by_range:
        assert area_scope is not None
        x_range, y_range, z_range = area_scope
...
def boxes3d_to_corners3d_torch(boxes3d, flip=False):
    ...
    """
    boxes_num = boxes3d.shape[0]
    h, w, l, ry = boxes3d[:, 3:4], boxes3d[:, 4:5], boxes3d[:, 5:
                                                            6], boxes3d[:, 6:7]
    if flip:
        ry = ry + np.pi
    centers = boxes3d[:, 0:3]
    ...
    x_corners = torch.cat(
        [l / 2., l / 2., -l / 2., -l / 2., l / 2., l / 2., -l / 2., -l / 2.],
        dim=1)  # (N, 8)
    y_corners = torch.cat([zeros, zeros, zeros, zeros, -h, -h, -h, -h],
                          dim=1)  # (N, 8)
    z_corners = torch.cat(
        [w / 2., -w / 2., -w / 2., w / 2., w / 2., -w / 2., -w / 2., w / 2.],
        dim=1)  # (N, 8)
    corners = torch.cat(
        (x_corners.unsqueeze(dim=1), y_corners.unsqueeze(dim=1),
         z_corners.unsqueeze(dim=1)),
        dim=1)  # (N, 3, 8)
    ...
    corners_rotated = corners_rotated + centers.unsqueeze(dim=2).expand(
        -1, -1, 8)
...
def boxes3d_to_bev_torch(boxes3d):
    """
    :param boxes3d: (N, 8) [x, y, z, h, w, l, ry, batch_id]
    :return:
        boxes_bev: (N, 6) [x1, y1, x2, y2, ry, batch_id]
    """
    boxes_bev = boxes3d.new(torch.Size((boxes3d.shape[0], 6)))
    ...
    boxes_bev[:, 5] = boxes3d[:, 7]
    return boxes_bev


def boxes3d_to_3d_torch(boxes3d):
    """
    :param boxes3d: (N, 8) [x, y, z, h, w, l, ry, batch_id]
    :return:
        boxes_3d_torch: (N, 8) [x1, y1, x2, y2, ry, x3, y3, batch_id]
    """
    boxes_3d_torch = boxes3d.new(torch.Size((boxes3d.shape[0], 8)))

    cu, cv = boxes3d[:, 0], boxes3d[:, 2]
    half_l, half_w = boxes3d[:, 5] / 2, boxes3d[:, 4] / 2
    boxes_3d_torch[:, 0], boxes_3d_torch[:, 1] = cu - half_l, cv - half_w
    boxes_3d_torch[:, 2], boxes_3d_torch[:, 3] = cu + half_l, cv + half_w
    boxes_3d_torch[:, 4] = boxes3d[:, 6]
    boxes_3d_torch[:, 5] = boxes3d[:, 1] - boxes3d[:, 3]
    boxes_3d_torch[:, 6] = boxes3d[:, 1]
    boxes_3d_torch[:, 7] = boxes3d[:, 7]
    return boxes_3d_torch
...
def enlarge_box3d_ratio(boxes3d, extra_ratio):
    """
    :param boxes3d: (N, 7) [x, y, z, h, w, l, ry]
    """
    if isinstance(boxes3d, np.ndarray):
        ...
        large_boxes3d = boxes3d.clone()
    large_boxes3d[:, 1] += large_boxes3d[:, 3] * extra_ratio * 0.5
    large_boxes3d[:, 3:6] *= 1 + extra_ratio
    return large_boxes3d
...
def get_iou3d(corners3d, query_corners3d, need_bev=False):
    ...
    for i in range(N):
        for j in range(M):
            ...
            if h_overlap == 0 and not need_bev:
                continue

            bottom_a, bottom_b = Polygon(A[i, 0:4, [0, 2]].T), Polygon(
                B[j, 0:4, [0, 2]].T)
