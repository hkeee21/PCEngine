...
__device__ inline float iou_bev(const float *box_a, const float *box_b){
    // params: box_a (6) [x1, y1, x2, y2, angle, batch_id]
    // params: box_b (6) [x1, y1, x2, y2, angle, batch_id]
    if (box_a[5] != box_b[5])
        return 0;
...
__device__ inline float iou_3d(const float *box_a, const float *box_b){
    // params: box_a (8) [x1, y1, x2, y2, angle, x3, y3, batch_id]
    // params: box_b (8) [x1, y1, x2, y2, angle, x3, y3, batch_id]
    if (box_a[7] != box_b[7])
        return 0;
    float va = (box_a[2] - box_a[0]) * (box_a[3] - box_a[1]) * (box_a[6] - box_a[5]);
    float vb = (box_b[2] - box_b[0]) * (box_b[3] - box_b[1]) * (box_b[6] - box_b[5]);
    float v_overlap = box_overlap(box_a, box_b) * fmaxf(fminf(box_a[6], box_b[6]) - fmaxf(box_a[5], box_b[5]), 0);
    return v_overlap / fmaxf(va + vb - v_overlap, 1e-8);
}
...
__global__ void boxes_iou_bev_kernel(const int num_a, const float *boxes_a, const int num_b, const float *boxes_b, float *ans_iou){
    ...
    const float * cur_box_a = boxes_a + a_idx * 6;
    const float * cur_box_b = boxes_b + b_idx * 6;
...
__global__ void boxes_iou_3d_kernel(const int num_a, const float *boxes_a, const int num_b, const float *boxes_b, float *ans_iou){
    ...
    const float * cur_box_a = boxes_a + a_idx * 8;
    const float * cur_box_b = boxes_b + b_idx * 8;
    float cur_iou_3d = iou_3d(cur_box_a, cur_box_b);
    ans_iou[a_idx * num_b + b_idx] = cur_iou_3d;
}


__global__ void correspond_boxes_iou_3d_kernel(const int num, const float *boxes_a, const float *boxes_b, float *ans_iou){
    ...
    const float * cur_box_a = boxes_a + idx * 8;
    const float * cur_box_b = boxes_b + idx * 8;
    float cur_iou_3d = iou_3d(cur_box_a, cur_box_b);
    ans_iou[idx] = cur_iou_3d;
}

__global__ void nms_kernel(const int boxes_num, const float nms_overlap_thresh,
                           const float *boxes, unsigned long long *mask){
    //params: boxes (N, 6) [x1, y1, x2, y2, ry, batch_id]
    ...
    __shared__ float block_boxes[THREADS_PER_BLOCK_NMS * 6];

    if (threadIdx.x < col_size) {
        block_boxes[threadIdx.x * 6 + 0] = boxes[(THREADS_PER_BLOCK_NMS * col_start + threadIdx.x) * 6 + 0];
        block_boxes[threadIdx.x * 6 + 1] = boxes[(THREADS_PER_BLOCK_NMS * col_start + threadIdx.x) * 6 + 1];
        block_boxes[threadIdx.x * 6 + 2] = boxes[(THREADS_PER_BLOCK_NMS * col_start + threadIdx.x) * 6 + 2];
        block_boxes[threadIdx.x * 6 + 3] = boxes[(THREADS_PER_BLOCK_NMS * col_start + threadIdx.x) * 6 + 3];
        block_boxes[threadIdx.x * 6 + 4] = boxes[(THREADS_PER_BLOCK_NMS * col_start + threadIdx.x) * 6 + 4];
        block_boxes[threadIdx.x * 6 + 5] = boxes[(THREADS_PER_BLOCK_NMS * col_start + threadIdx.x) * 6 + 5];
    }
    __syncthreads();

    if (threadIdx.x < row_size) {
        const int cur_box_idx = THREADS_PER_BLOCK_NMS * row_start + threadIdx.x;
        const float *cur_box = boxes + cur_box_idx * 6;
        ...
        for (i = start; i < col_size; i++) {
            if (iou_bev(cur_box, block_boxes + i * 6) > nms_overlap_thresh){
                t |= 1ULL << i;
            }
        }
...
__global__ void nms_3d_kernel(const int boxes_num, const float nms_overlap_thresh,
                           const float *boxes, unsigned long long *mask){
    //params: boxes (N, 8) [x1, y1, x2, y2, ry, x3, y3, batch_id]
    //params: mask (N, N/THREADS_PER_BLOCK_NMS)
    ...
    __shared__ float block_boxes[THREADS_PER_BLOCK_NMS * 8];

    if (threadIdx.x < col_size) {
        block_boxes[threadIdx.x * 8 + 0] = boxes[(THREADS_PER_BLOCK_NMS * col_start + threadIdx.x) * 8 + 0];
        block_boxes[threadIdx.x * 8 + 1] = boxes[(THREADS_PER_BLOCK_NMS * col_start + threadIdx.x) * 8 + 1];
        block_boxes[threadIdx.x * 8 + 2] = boxes[(THREADS_PER_BLOCK_NMS * col_start + threadIdx.x) * 8 + 2];
        block_boxes[threadIdx.x * 8 + 3] = boxes[(THREADS_PER_BLOCK_NMS * col_start + threadIdx.x) * 8 + 3];
        block_boxes[threadIdx.x * 8 + 4] = boxes[(THREADS_PER_BLOCK_NMS * col_start + threadIdx.x) * 8 + 4];
        block_boxes[threadIdx.x * 8 + 5] = boxes[(THREADS_PER_BLOCK_NMS * col_start + threadIdx.x) * 8 + 5];
        block_boxes[threadIdx.x * 8 + 6] = boxes[(THREADS_PER_BLOCK_NMS * col_start + threadIdx.x) * 8 + 6];
        block_boxes[threadIdx.x * 8 + 7] = boxes[(THREADS_PER_BLOCK_NMS * col_start + threadIdx.x) * 8 + 7];
    }
    __syncthreads();

    if (threadIdx.x < row_size) {
        const int cur_box_idx = THREADS_PER_BLOCK_NMS * row_start + threadIdx.x;
        const float *cur_box = boxes + cur_box_idx * 8;
        ...
        for (i = start; i < col_size; i++) {
            if (iou_3d(cur_box, block_boxes + i * 8) > nms_overlap_thresh){
                t |= 1ULL << i;
            }
        }
        const int col_blocks = DIVUP(boxes_num, THREADS_PER_BLOCK_NMS);
...
void correspondBoxesiou3dLauncher(const int num, const float *boxes_a, const float *boxes_b, float *ans_iou){

    dim3 blocks(DIVUP(num, THREADS_PER_BLOCK * THREADS_PER_BLOCK));
    dim3 threads(THREADS_PER_BLOCK * THREADS_PER_BLOCK);
    
    correspond_boxes_iou_3d_kernel<<<blocks, threads>>>(num, boxes_a, boxes_b, ans_iou);
}
