...
load_list = [
    os.path.join(pd, 'src', 'roiaware_pool3d.cpp'),
    os.path.join(pd, 'src', 'roiaware_pool3d_kernel.cu')
]
roiaware_pool3d_cuda = load('roiaware_pool3d', load_list, verbose=False)
...
def points_in_boxes_gpu(points, boxes):
    ...
    roiaware_pool3d_cuda.points_in_boxes_gpu(boxes.contiguous(),
                                             points.contiguous(),
                                             box_idxs_of_pts)
...
class RoIAwarePool3dFunction(Function):
    ...
    def forward(ctx, rois, pts, pts_feature, out_size, max_pts_each_voxel,
        ...
        pts_idx_of_voxels = pts_feature.new_zeros(
            (num_rois, out_x, out_y, out_z, max_pts_each_voxel),
            dtype=torch.int)
    ...
    def backward(ctx, grad_out):
        ...
        roiaware_pool3d_cuda.backward(pts_idx_of_voxels, argmax,
