...
def tune(chns: int, ks: int, vs: float):
    ...
    tune_params["_MPNS_PER_BLOCK"] = [2**i for i in range(0, 9)]
    ...
    res, _ = tune_kernel("gather_all_input_major_csr_balance", "tune.cu", problem_size,
        args, tune_params, grid_div_x=grid_div_x, grid_div_y=grid_div_y,
        restrictions=restrict, verbose=True, lang="CUDA", iterations=100)
