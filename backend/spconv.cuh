...
__global__ void gather_all_input_major_csr_float_4(
    const int nnz, const int c_in, float *in_f, const int *__restrict__ kpos,
    const int *__restrict__ icsr, const int *__restrict__ imap, float *g_f) {
  // id-th nnz
...
#pragma unroll
    for (int c = 0;; c += blockDim.x) {
      ...
      _FLOAT2(g_f[buf_pos * c_in + cp]) = _FLOAT2(in_f[id * c_in + cp]);
    }
  }
}
...
#pragma unroll
    for (int c = 0;; c += blockDim.x) {
      ...
      _FLOAT2(g_f[buf_pos * c_mod + cp]) =
          (cp < c_in) ? _FLOAT2(in_f[id * c_in + cp]) : _FLOAT2(pad[0]);
    }
  }
}

__global__ void gather_all_input_major_csr_half_8(
    const int nnz, const int c_in, half *in_f, const int *__restrict__ kpos,
    const int *__restrict__ icsr, const int *__restrict__ imap, half *g_f) {
  // id-th nnz
  const int id = blockIdx.x * blockDim.z + threadIdx.z;
  if (id >= nnz) {
    return;
...
#pragma unroll
    for (int c = 0;; c += blockDim.x) {
      ...
      _HALF2(g_f[buf_pos * c_in + cp]) = _HALF2(in_f[id * c_in + cp]);
    }
  }
}
...
#pragma unroll
    for (int c = 0;; c += blockDim.x) {
      ...
      _HALF2(g_f[buf_pos * c_in + cp]) =
          (cp < c_in) ? _HALF2(in_f[id * c_in + cp]) : _HALF2(pad[0]);
    }
  }
}
...
#pragma unroll
    for (int c = 0;; c += blockDim.x) {
      ...
      g_f[buf_pos * c_in + cp] = in_f[id * c_in + cp];
    }
  }
}
...
__global__ void scatter_all_output_major_csr_float(
    const int nnz, const int c_out, float *s_f, const int *__restrict__ kpos,
    const int *__restrict__ ocsr, const int *__restrict__ omap, float *out_f) {
  // id-th nnz
...
#pragma unroll
    for (int k = m_start; k < m_end; k++) {
      ...
      int buf_pos = buf_start + buf_ofs;
      _FLOAT4(acc[0]) =
          addFLOAT4(_FLOAT4(acc[0]), _FLOAT4(s_f[buf_pos * c_out + cp]));
    }
...
__global__ void scatter_all_output_major_csr_half(
    const int nnz, const int c_out, half *s_f, const int *__restrict__ kpos,
    const int *__restrict__ ocsr, const int *__restrict__ omap, half *out_f) {
  // id-th nnz
  const int id = blockIdx.x * blockDim.y + threadIdx.y;
  if (id >= nnz) {
    return;
...
__global__ void gemm_float_fused_unaligned_2(
  ...
  // Coordinate. x is for rows, y is for columns.
  const int cx = BLOCK_SIZE * bx + ctx;
  const int y = BLOCK_SIZE * by + ty - __ldg(&qkpos[widx]) + __ldg(&kpos[widx]);
...
template <int BLOCK_SIZE, int N_LOOP, int SKEW>
__global__ void
naive_gemm_fp16_2(const int *__restrict__ kpos, const int *__restrict__ qkpos,
                  const int k_vol, const int c_in, const int c_out,
...
template <int BLOCK_SIZE, int N_LOOP, int SKEW>
__global__ void
naive_gemm_fp32_2(const int *__restrict__ kpos, const int *__restrict__ qkpos,
                  const int k_vol, const int c_in, const int c_out,
...
#pragma unroll
  ...
  for (int n = 0; n < N_LOOP; n++) {
    ...
    if (ctx < c_in && in_row > -1) {
      cuda::memcpy_async(&As[n][ty][ctx], &in_f[c_in * in_row + ctx], shape4,
                         pipe);
    } else {
      cuda::memcpy_async(&As[n][ty][ctx], &padding[0], shape4, pipe);
    }
...
template <int WARP_ROW_TILES, int WARP_COL_TILES>
__global__ void wmma_ragged_gemm_fp16_kernel(int kvol, half *inputBuffer,
                                             half *weight, half *outputBuffer,
                                             int *ragged_ptr, int in_channel,
                                             int out_channel) {
  // index
  int tx = threadIdx.x;
  int ty = threadIdx.y;
  ...
  int warpM = (bx * blockDim.x + tx) / WARP_SIZE * WARP_ROW_TILES;
  int warpN = (by * blockDim.y + ty) * WARP_COL_TILES; // add y dimension

  // Declare the fragments
  wmma::fragment<wmma::accumulator, WMMA_M, WMMA_N, WMMA_K, half>
      acc_frag[WARP_ROW_TILES][WARP_COL_TILES];
#pragma unroll
  for (int rr = 0; rr < WARP_ROW_TILES; rr++) {
#pragma unroll
    for (int cc = 0; cc < WARP_COL_TILES; cc++) {
      wmma::fill_fragment(acc_frag[rr][cc], __float2half(0.0f));
    }
  }
  // Loop over k
  for (int i = 0; i < in_channel; i += WMMA_K) {
    wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K, half,
                   wmma::row_major>
        a_frag[WARP_ROW_TILES];
    wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_N, WMMA_K, half,
                   wmma::row_major>
        b_frag[WARP_COL_TILES];
    for (int rr = 0; rr < WARP_ROW_TILES; rr++) {
      int input_row = (warpM + rr) * WMMA_M;
      int input_col = i;
      int weight_row_ragged_offset =
          binary_search(ragged_ptr, (warpM + rr) * WMMA_M, 0, kvol) *
          in_channel;
      int weight_row = i + weight_row_ragged_offset;
      // Load the inputs
      wmma::load_matrix_sync(a_frag[rr],
                             inputBuffer + input_row * in_channel + input_col,
                             in_channel);
      for (int cc = 0; cc < WARP_COL_TILES; cc++) {
        int weight_col = (warpN + cc) * WMMA_N;
        wmma::load_matrix_sync(b_frag[cc],
                               weight + weight_row * out_channel + weight_col,
                               out_channel);
        // Perform the matrix multiplication
        wmma::mma_sync(acc_frag[rr][cc], a_frag[rr], b_frag[cc],
                       acc_frag[rr][cc]);
      }
    }
  }
#pragma unroll
  for (int rr = 0; rr < WARP_ROW_TILES; rr++) {
    int output_row = (warpM + rr) * WMMA_M;
#pragma unroll
    for (int cc = 0; cc < WARP_COL_TILES; cc++) {
      int output_col = (warpN + cc) * WMMA_N;
      wmma::store_matrix_sync(
          outputBuffer + output_col + output_row * out_channel,
          acc_frag[rr][cc], out_channel, wmma::mem_row_major);
    }
  }
}

void wmma_ragged_gemm_fp16_wrapper(int k_vol, int qsum_nnz, int buffer_offset,
                                   half *kernel, half *buffer, int *kpos_ptr,
                                   int *qkpos_ptr, int in_channel,
                                   int out_channel) {
  dim3 gridDim;
  dim3 blockDim;
  blockDim.x = 128;
  ...
  if (in_channel >= 32 && out_channel < 64) {
    int WARP_ROW_TILES = 1;
    int WARP_COL_TILES = 1;
    gridDim.x =
        CEIL(qsum_nnz, WARP_ROW_TILES * WMMA_M * blockDim.x / WARP_SIZE);
    gridDim.y = CEIL(out_channel, WARP_COL_TILES * WMMA_N * blockDim.y);
    // printf("gridDim.x %d gridDim.y %d\n", gridDim.x, gridDim.y);
    wmma_ragged_gemm_fp16_kernel<1, 1>
        <<<gridDim, blockDim>>>(k_vol, buffer, kernel, buffer + buffer_offset,
                                qkpos_ptr, in_channel, out_channel);
    // simple_wmma_ragged_gemm_fp16_kernel<<<gridDim, blockDim>>>(
    //     k_vol, buffer, weight, buffer + buffer_offset, qkpos_ptr, in_channel,
    //     out_channel);
  } else if (in_channel >= 32 && out_channel >= 64) {
    int WARP_ROW_TILES = 1;
    ...
    gridDim.y = CEIL(out_channel, WARP_COL_TILES * WMMA_N * blockDim.y);
    // printf("gridDim.x %d gridDim.y %d\n", gridDim.x, gridDim.y);
    wmma_ragged_gemm_fp16_kernel<1, 2>
        <<<gridDim, blockDim>>>(k_vol, buffer, kernel, buffer + buffer_offset,
                                qkpos_ptr, in_channel, out_channel);
  } else if (in_channel >= 16 && out_channel >= 16) {
    blockDim.x = 128;
    blockDim.y = 1;
    int WARP_ROW_TILES = 1;
    int WARP_COL_TILES = 1;
    ...
    naive_gemm_fp16_2<32, 4, 8>
        <<<dim3(CEIL(out_channel, 32), CEIL(qsum_nnz, 128), 1),
           dim3(16, 32, 1)>>>(kpos_ptr, qkpos_ptr, k_vol, in_channel,
                              out_channel, buffer, kernel,
