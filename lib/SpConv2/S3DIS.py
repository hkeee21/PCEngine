...
class S3DISDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        pcs, feats = current_points[:, 6:9], current_points[:, 2:6]
        pcs -= np.min(pcs, axis=0, keepdims=True)
        pcs, inds = sparse_quantize(pcs, voxel_size=0.005, return_index=True)
        input_nnz = pcs.shape[0]
        coords = np.zeros((input_nnz, 4))
        coords[:, 1:] = pcs
        coords[:, 0] = 0

        shape = np.max(coords, axis=0) + 10
        shape = shape[1:]

        coords = torch.as_tensor(coords, dtype=torch.int)
        feats = torch.as_tensor(feats[inds], dtype=torch.float)
        labels = torch.as_tensor(current_labels[inds], dtype=torch.int)
