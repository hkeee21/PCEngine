...
def fapply(input: spTensor, fn: Callable[..., torch.Tensor], *args,
    ...
    output = spTensor(coords=input.coords, feats=feats, buffer=input.buffer, \
        ...
        coords_min=input.coords_min, coords_max=input.coords_max)
...
def sparse_collate(inputs: List[spTensor]) -> spTensor:
    ...
    # coords_min = torch.min(coords[:, 1:], dim=0).values.cpu().numpy().tolist()
    coords_min = [0, 0, 0]
...
def vanillaConv(
    ...
    # coords_min = [0, 0, 0]
    coords_min = torch.min(in_c[:, 1:], dim=0).values.numpy().tolist()
    coords_max = ((torch.max(in_c[:, 1:], dim=0).values.numpy() + 2 * np.array(pad) 
                  - (np.array(ks) - 1)) // np.array(stride)).tolist() 
    ...
    if ((stride[0] == 1 or stride[0] == ks[0])
        ...
        for i in range(nnz):
            for x in range(ks[0]):
                if (in_c[i, 1] - kofs_x[x] + pad[0]) % stride[0] != 0 or \
                    ((in_c[i, 1] - kofs_x[x] + pad[0]) // stride[0]) < coords_min[0] or \
                    ((in_c[i, 1] - kofs_x[x] + pad[0]) // stride[0]) > coords_max[0]:
                    continue
                for y in range(ks[1]):
                    if (in_c[i, 2] - kofs_y[y] + pad[1]) % stride[1] != 0 or \
                        ((in_c[i, 2] - kofs_y[y] + pad[1]) // stride[1]) < coords_min[1] or \
                        ((in_c[i, 2] - kofs_y[y] + pad[1]) // stride[1]) > coords_max[1]:
                        continue
                    for z in range(ks[2]):
                        if (in_c[i, 3] - kofs_y[z] + pad[2]) % stride[2] != 0 or \
                            ((in_c[i, 3] - kofs_y[z] + pad[2]) // stride[2]) < coords_min[2] or \
                            ((in_c[i, 3] - kofs_y[z] + pad[2]) // stride[2]) > coords_max[2]:
                            continue
                        out_c[out_nnz, 0] = in_c[i, 0]
                        out_c[out_nnz, 1] = ((in_c[i, 1] - kofs_x[x] + pad[0]) // stride[0])
                        out_c[out_nnz, 2] = ((in_c[i, 2] - kofs_y[y] + pad[1]) // stride[1])
                        out_c[out_nnz, 3] = ((in_c[i, 3] - kofs_y[z] + pad[2]) // stride[2])
                        out_nnz += 1
    ...
    for i in range(nnz):
        ...
        for j in range(out_nnz):
            if in_c[i, 0] == out_c[j, 0]:
                off_x = in_c[i, 1] - out_c[j, 1] * stride[0] + pad[0]
                off_y = in_c[i, 2] - out_c[j, 2] * stride[1] + pad[1]
                off_z = in_c[i, 3] - out_c[j, 3] * stride[2] + pad[2]
